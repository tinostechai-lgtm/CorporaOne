...
class UserController extends Controller
{
    ...
    public function store(Request $request)
    {
        if (\Auth::user() && \Auth::user()->can('create user')) {
            $objUser = \Auth::user() ? \Auth::user()->creatorId() : null;

            if (!$objUser) {
                return redirect()->back()->with('error', __('User creator ID is not found.'));
            }

            $default_language = DB::table('settings')
                ->select('value')
                ->where('name', 'default_language')
                ->where('created_by', '=', $objUser)
                ->first();

            $language = !empty($default_language) ? $default_language->value : 'en';
            ...
            if (\Auth::user()->type == 'super admin') {
                ...
                $enableLogin = !empty($request->password_switch) && $request->password_switch == 'on' ? 1 : 0;
                ...
                if (!$plan) {
                    return redirect()->back()->with('error', __('No plan found.'));
                }
                ...
                $user->lang = $language;
                $user->referral_code = $code;
                $user->created_by = $objUser;
                $user->email_verified_at = (Utility::settings()['email_verification'] == 'on') ? null : now();
                $user->is_enable_login = $enableLogin;
                ...
                $objUser = User::find($objUser);
                if (!$objUser) {
                    return redirect()->back()->with('error', __('Creator user not found.'));
                }
                ...
                if (!$plan) {
                    return redirect()->back()->with('error', __('User plan not found.'));
                }
                ...
                if ($total_user < $plan->max_users || $plan->max_users == -1) {
                    ...
                    if ($request['type'] != 'client') {
                        Utility::employeeDetails($user->id, $objUser->id);
                    }
                } else {
                    return redirect()->back()->with('error', __('Your user limit is over, Please upgrade plan.'));
                }
    ...
    public function editprofile(Request $request)
{
    ...
    if ($request->hasFile('profile')) {
        ...
        $fileNameToStore = time() . '_' . preg_replace('/[^a-zA-Z0-9]/', '_', $user->name) . '.' . $extension;
        ...
        // Move the file directly to public/uploads/avatar/
        $file->move($destinationPath, $fileNameToStore);
        
        // Update user's avatar field with just the filename
        $user->avatar = $fileNameToStore;
        
        // Also update the profile field for consistency
        $user->profile = asset('uploads/avatar/' . $fileNameToStore);
    }
    ...
    return redirect()->route('profile', $user)->with('success', 'Profile successfully updated.');
}
