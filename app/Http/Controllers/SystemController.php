...
class SystemController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (\Auth::user()->can('manage system settings')) {
            ...
            if ($request->filled('webhook_notes')) {
                \DB::insert(
                    'insert into settings (`value`, `name`,`created_by`) values (?, ?, ?) ON DUPLICATE KEY UPDATE `value` = VALUES(`value`) ',
                    [
                        $request->webhook_notes,
                        'webhook_notes',
                        \Auth::user()->creatorId(),
                    ]
                );
    ...
    public function saveCompanySettings(Request $request)
{
    if (\Auth::user()->can('manage company settings')) {
        ...
        // ✅ ADD: Week Off Settings
        if (isset($request->week_off_days) && is_array($request->week_off_days)) {
            $post['week_off_days'] = implode(',', $request->week_off_days);
        } else {
            $post['week_off_days'] = '';
        }
        
        // ✅ ADD: Holiday Settings
        if (isset($request->company_holidays) && is_array($request->company_holidays)) {
            $post['company_holidays'] = json_encode($request->company_holidays);
        } else {
            $post['company_holidays'] = '[]';
        }
        
        // ✅ ADD: Custom Holiday Calendar
        if (isset($request->custom_holidays) && is_array($request->custom_holidays)) {
            $post['custom_holidays'] = json_encode($request->custom_holidays);
        } else {
            $post['custom_holidays'] = '[]';
        }
        
        // ✅ ADD: Week Off Type (Fixed/Rotating)
        if (isset($request->week_off_type)) {
            $post['week_off_type'] = $request->week_off_type;
        } else {
            $post['week_off_type'] = 'fixed';
        }
        
        // ✅ ADD: Employee specific week off (if needed)
        if (isset($request->employee_week_offs) && is_array($request->employee_week_offs)) {
            $post['employee_week_offs'] = json_encode($request->employee_week_offs);
        } else {
            $post['employee_week_offs'] = '[]';
        }

        // Existing code for VAT and IP restrict
...
if ($request->company_favicon) {
    ...
    public function companyIndex(Request $request)
    {
        if (\Auth::user()->can('manage company settings')) {
            ...
            // $comSetting = DB::table('settings')->where('created_by', '=', \Auth::user()->id)->pluck('name','value')->toArray();
    ...
    public function CookieConsent(Request $request)
    {
        ...
        if ($settings['enable_cookie'] == "on" && $settings['cookie_logging'] == "on") {
            ...
            if (!file_exists(public_path() . '/uploads/sample/data.csv')) {
                ...
                file_put_contents(public_path() . '/uploads/sample/data.csv', $first_line . PHP_EOL, FILE_APPEND | LOCK_EX);
            }
            file_put_contents(public_path() . '/uploads/sample/data.csv', $new_line . PHP_EOL, FILE_APPEND | LOCK_EX);
    ...
    /**
 * Get company week off settings
 */
public function getWeekOffSettings()
{
    $settings = Utility::settings();
    
    return [
        'week_off_days' => isset($settings['week_off_days']) ? explode(',', $settings['week_off_days']) : [],
        'week_off_type' => isset($settings['week_off_type']) ? $settings['week_off_type'] : 'fixed',
        'employee_week_offs' => isset($settings['employee_week_offs']) ? json_decode($settings['employee_week_offs'], true) : [],
    ];
}
...
public function getHolidaySettings()
{
    ...
    return [
        'company_holidays' => isset($settings['company_holidays']) ? json_decode($settings['company_holidays'], true) : [],
        'custom_holidays' => isset($settings['custom_holidays']) ? json_decode($settings['custom_holidays'], true) : [],
    ];
}

/**
 * Check if a date is a week off for a specific employee
 */
public function isWeekOff($employeeId, $date)
{
    $settings = Utility::settings();
    $dayOfWeek = date('N', strtotime($date)); // 1=Monday, 7=Sunday
    
    // Check employee-specific week off first
    if (isset($settings['employee_week_offs'])) {
        $employeeWeekOffs = json_decode($settings['employee_week_offs'], true);
        if (isset($employeeWeekOffs[$employeeId]) && in_array($dayOfWeek, $employeeWeekOffs[$employeeId])) {
            return true;
        }
    }
    
    // Check company-wide week off
    if (isset($settings['week_off_days'])) {
        $weekOffDays = explode(',', $settings['week_off_days']);
        if (in_array($dayOfWeek, $weekOffDays)) {
            return true;
        }
    }
...
public function isHoliday($date)
{
    ...
    if (isset($settings['company_holidays'])) {
        $holidays = json_decode($settings['company_holidays'], true);
        foreach ($holidays as $holiday) {
            if (isset($holiday['date']) && $holiday['date'] == $dateFormatted) {
                return true;
            }
        }
    }
    ...
    if (isset($settings['custom_holidays'])) {
        $customHolidays = json_decode($settings['custom_holidays'], true);
...
/**
 * Get all holidays for a specific year/month
 */
public function getHolidaysForMonth($year, $month)
{
    $settings = Utility::settings();
    $holidays = [];
    ...
    if (isset($settings['company_holidays'])) {
        $companyHolidays = json_decode($settings['company_holidays'], true);
        foreach ($companyHolidays as $holiday) {
            if (isset($holiday['date']) && date('Y-m', strtotime($holiday['date'])) == $year . '-' . $month) {
                $holidays[] = $holiday;
            }
        }
    }
