...
class BillController extends Controller
{
    ...
    /**
     * Get Chart of Accounts filtered by plan
     */
    private function getFilteredChartAccounts()
    {
        ...
        $accounts = $query->orderBy('code')->get()->pluck('code_name', 'id');
    ...
    public function create($vendorId = 0)
    {
        if(\Auth::user()->can('create bill'))
        {
            ...
            // Get filtered Chart of Accounts based on plan
            $chartAccounts = $this->getFilteredChartAccounts();
    ...
    public function store(Request $request)
    {
        if(\Auth::user()->can('create bill'))
        {
            ...
            // Validate that selected accounts are enabled in the plan
            $enabledAccountIds = $this->getEnabledAccountIds();
            if (!empty($enabledAccountIds) && !empty($request->items)) {
                foreach ($request->items as $item) {
                    if (!empty($item['chart_account_id']) && !in_array($item['chart_account_id'], $enabledAccountIds)) {
...
$bill_products = BillProduct::where('bill_id', $bill->id)->get();
                foreach ($bill_products as $bill_product) {
                    $product = ProductService::find($bill_product->product_id);
                    if (!$product || empty($product->expense_chartaccount_id)) {
                        \Log::warning('Bill #' . $bill->bill_id . ' skipping product #' . $bill_product->product_id . ' - no expense account');
                        continue;
                    }
...
$bill_accounts = BillAccount::where('ref_id', $bill->id)->get();
                foreach ($bill_accounts as $bill_product) {
                    if (empty($bill_product->chart_account_id)) {
                        \Log::warning('Bill #' . $bill->bill_id . ' skipping BillAccount - no chart_account_id');
                        continue;
                    }
    ...
    public function resent($id)
    {
        ...
        if($bill->created_by == \Auth::user()->creatorId())
        {
            ...
            if($settings['bill_resent'] == 1)
            {
                ...
                return redirect()->back()->with('success', __('Bill successfully resent.') . (($resp['is_success'] == false && !empty($resp['error'])) ? '<br> <span class="text-danger">' . $resp['error'] . '</span>' : ''));
            }
            
            return redirect()->back()->with('success', __('Bill successfully resent.'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
        ...
        if(\Auth::user()->can('create payment bill'))
        {
            ...
            if(!empty($request->add_receipt))
            {
                ...
                if($result==1)
                {
                    ...
                    {
                        $path = public_path('uploads/payment' . $billPayment->add_receipt);
                        if(file_exists($path))
                        {
                            \File::delete($path);
                        }
