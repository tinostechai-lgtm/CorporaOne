...
class EmployeeController extends Controller
{
            ...
            if($request->hasFile('document'))
            {
                foreach($request->file('document') as $key => $document)
                {
                    ...
                    try {
                        ...
                        return redirect()->back()->with('error', __('Failed to upload document: ') . $e->getMessage());
                    }
                }
            }
                ...
                foreach($request->file('document') as $key => $document)
                {
                    if(!empty($document))
                    {
                        ...
                        try {
                            ...
                            {
                                $oldFilePath = 'uploads/document/' . $employee_document->document_value;
    ...
    public function destroy($id)
    {
        if(Auth::user()->can('delete employee'))
        {
            ...
            {
                $filePath = 'uploads/document/' . $emp_document->document_value;
    ...
    {
        $currantLang = \Cookie::get('LANGUAGE') ?? 'en';
    ...
    public function downloadDocument($id)
    {
        if (\Auth::user()->can('view employee')) {
            try {
                Log::debug('downloadDocument method called', ['document_id' => $id, 'user_id' => Auth::user()->id]);

                $employeeDocument = EmployeeDocument::findOrFail($id);
                $filePath = 'uploads/document/' . $employeeDocument->document_value;
                $fullPath = Storage::disk('public')->path($filePath);

                Log::info('Attempting to download document', [
                    'document_id' => $id,
                    'document_value' => $employeeDocument->document_value,
                    'file_path' => $fullPath,
                ...
                if (Storage::disk('public')->exists($filePath)) {
                    ...
                    return Storage::disk('public')->download($filePath, $employeeDocument->document_value, [
                ...
                Log::error('File not found for download', [
                    'document_id' => $id,
                    'file_path' => $fullPath,
                    'directory_exists' => Storage::disk('public')->exists('uploads/document'),
                    'directory_contents' => Storage::disk('public')->exists('uploads/document') ? Storage::disk('public')->files('uploads/document') : [],
                ]);

                return redirect()->back()->with('error', __('File not found: ') . $employeeDocument->document_value . '. Check storage/app/public/uploads/document/ directory.');
            } catch (\Exception $e) {
