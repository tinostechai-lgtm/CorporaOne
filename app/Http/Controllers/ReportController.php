...
//        dd($request->all());
    ...
    // ... (keep all your existing methods: incomeSummary, expenseSummary, etc.)

    /**
     * Account Statement Report (existing method)
     */
public function accountStatement(Request $request)
{
    $startMonth = $request->input('start_month', date('Y-m', strtotime("-5 month")));
    ...
    $query = TransactionLines::whereBetween('date', [$startDate, $endDate])
        ->where('created_by', \Auth::user()->creatorId());
    
    // IMPORTANT: Apply account filter if specified
    ...
    // Get all bank accounts for reference
    ...
    // Also get the selected account details for display
    $selectedAccountDetails = null;
    if ($accountId && $accountId != '' && $accountId != 'null') {
        if (isset($bankAccounts[$accountId])) {
            $selectedAccountDetails = $bankAccounts[$accountId];
        } elseif (isset($chartAccounts[$accountId])) {
            $selectedAccountDetails = $chartAccounts[$accountId];
        }
    }
    ...
    foreach ($transactions as $txn) {
        ...
        // Get account name - Try multiple sources with better fallbacks
        $accountNameText = 'N/A';
        $hasValidAccount = false;
        
        // First try to get from bank accounts
        if ($txn->account_id && isset($bankAccounts[$txn->account_id])) {
            $account = $bankAccounts[$txn->account_id];
            if ($account->holder_name == 'Cash') {
                $accountNameText = $account->holder_name;
            } elseif (empty($account->holder_name)) {
                $accountNameText = __('Stripe / Paypal');
            } else {
                $accountNameText = $account->holder_name . ($account->bank_name ? ' - ' . $account->bank_name : '');
            }
            $hasValidAccount = true;
        }
        // If not found in bank accounts, try chart of accounts
        elseif ($txn->account_id && isset($chartAccounts[$txn->account_id])) {
            $account = $chartAccounts[$txn->account_id];
            $accountNameText = $account->name . ($account->code ? ' (' . $account->code . ')' : '');
            $hasValidAccount = true;
        }
        // If still not found but has account_id, show the ID and description type
        elseif ($txn->account_id) {
            $descriptionType = $txn->reference_type ?? $txn->description ?? 'Transaction';
            $accountNameText = $descriptionType . ' (Account #' . $txn->account_id . ')';
            $hasValidAccount = true;
        }
        // If no account_id, use the description or reference type
        else {
            $accountNameText = $txn->reference_type ?? $txn->description ?? 'General Transaction';
            $hasValidAccount = false;
        }
        ...
        $ledgerTransactions->push((object)[
            ...
            'has_valid_account' => $hasValidAccount,
    ...
    // Get all bank accounts with their balances for the summary cards
    $allAccounts = BankAccount::where('created_by', \Auth::user()->creatorId())
        ->get()
        ->map(function($account) use ($startDate, $endDate) {
            $totalCredit = TransactionLines::where('account_id', $account->id)
                ->whereBetween('date', [$startDate, $endDate])
                ->sum('credit');
            $totalDebit = TransactionLines::where('account_id', $account->id)
                ->whereBetween('date', [$startDate, $endDate])
                ->sum('debit');
            $account->total = $totalCredit - $totalDebit;
            return $account;
        });
    
    // Also add chart of accounts that have transactions but not in bank_accounts
    $accountIdsWithTransactions = $transactions->pluck('account_id')->unique()->filter();
    $chartAccountsWithTransactions = ChartOfAccount::where('created_by', \Auth::user()->creatorId())
        ->whereIn('id', $accountIdsWithTransactions)
        ->get()
        ->map(function($account) use ($startDate, $endDate) {
            $totalCredit = TransactionLines::where('account_id', $account->id)
                ->whereBetween('date', [$startDate, $endDate])
            ...
            $totalDebit = TransactionLines::where('account_id', $account->id)
                ...
                ->sum('debit');
            $account->total = $totalCredit - $totalDebit;
            $account->holder_name = $account->name;
            $account->bank_name = '';
            return $account;
        });
    
    // Merge both collections
    $allAccounts = $allAccounts->merge($chartAccountsWithTransactions);
    
    // Split accounts into revenue and payment based on balance
    $revenueAccounts = $allAccounts->filter(function($account) {
        return $account->total > 0;
    });
    
    $paymentAccounts = $allAccounts->filter(function($account) {
        return $account->total < 0;
    });
    ...
    $reportData = [
        'summary' => [
            ...
            'total_transactions' => $ledgerTransactions->count(),
            'end_balance' => $runningBalance,
        ],
        'ledgerTransactions' => $ledgerTransactions,
        'revenueAccounts' => $revenueAccounts,
        'paymentAccounts' => $paymentAccounts,
        'selected_account' => $selectedAccountDetails,
        'filtered_account_id' => $accountId,
        'filtered_account_name' => $accountName,
    ];
    
    // Prepare filter data for display
    $filter = [
        'startDateRange' => date('M Y', strtotime($startDate)),
        'endDateRange' => date('M Y', strtotime($endDate)),
        'type' => $type == 'all' ? __('All') : ucfirst($type),
        'account' => $accountId ? ($accountName ?: ($selectedAccountDetails->holder_name ?? $selectedAccountDetails->name ?? 'Selected Account')) : __('All'),
    ];
    
    $types = [
        'all' => __('All'),
        'debit' => __('Debit Only'),
        'credit' => __('Credit Only'),
    ];
    
    return view('report.statement_report', compact('reportData', 'filter', 'types'));
}
   // ==============================================
    // NEW: BANK STATEMENT UPLOAD & COMPARISON METHODS
    // ==============================================
     ...
     */
/**
 * Bank Statement vs Ledger Comparison - Main Entry Point
 */
public function bankStatementComparison(Request $request)
{
    ...
    $bankAccounts = BankAccount::where('created_by', \Auth::user()->creatorId())
        ...
        ->mapWithKeys(function ($account) {
            return [
                $account->id => $account->holder_name . ' (Account #' . $account->id . ')'
            ];
        });
    $bankAccounts->prepend(__('Select Bank Account'), '');
    ...
    $comparison = null;
    $ledgerPreview = collect();

    if ($submissionId && $accountId) {
        $submission = BankStatementSubmission::where('id', $submissionId)
            ->where('created_by', \Auth::user()->creatorId())
            ->firstOrFail();
        
        $ledgerTransactions = $this->getLedgerTransactionsByDateRange(
        ...
        $comparison->submission_id = $submission->id;
        $ledgerPreview = $ledgerTransactions->take(10);
    } elseif ($accountId && $accountId != '') {
        // Preview transactions for selected account (no submission needed)
        $ledgerPreview = $this->getLedgerTransactionsByDateRange(
            $startDate, $endDate, $accountId
        )->take(10);
    }

    return view('report.bank-statement-comparison', compact(
        'bankAccounts', 
        'comparison',
        'submissionId', 
        'accountId',
...
/**
 * Export Bank vs Ledger Comparison
 */
public function exportComparison($submissionId, Request $request)
{
    $submission = BankStatementSubmission::where('id', $submissionId)
        ->where('created_by', \Auth::user()->creatorId())
        ->firstOrFail();
    ...
    // Create CSV
    $filename = sprintf('bank_ledger_comparison_%s_%s.csv', 
        $submission->id, now()->format('Y-m-d_H-i')
    );
    ...
    $callback = function() use ($comparison, $submission, $startDate, $endDate, $accountId) {
        $file = fopen('php://output', 'w');
        
        // Headers
        fputcsv($file, ['Bank Statement vs Ledger Reconciliation']);
        fputcsv($file, ['Statement ID', $submission->id]);
        fputcsv($file, ['Bank', $submission->bank_name ?? 'N/A']);
        fputcsv($file, ['Account', BankAccount::find($accountId)?->holder_name ?? 'All']);
        ...
        fputcsv($file, ['Total Bank TXNs', $comparison->total_bank_txs]);
        fputcsv($file, ['Total Ledger TXNs', $comparison->total_ledger_txs]);
        fputcsv($file, ['Matched', $comparison->matched_count . ' (' . $comparison->match_accuracy . '%)']);
        fputcsv($file, ['Bank Only', $comparison->unmatched_bank]);
        fputcsv($file, ['Ledger Only', $comparison->unmatched_ledger]);
        fputcsv($file, ['Total Variance', '$' . number_format($comparison->total_variance, 2)]);
        fputcsv($file, []);
        
        // Comparison Table
        fputcsv($file, [
            'Status', 'Date', 'Bank Desc', 'Bank Amt', 
            'Ledger Date', 'Ledger Desc', 'Ledger Amt', 
            'Variance', 'Bank Ref', 'Ledger Ref'
        ]);
        
        foreach ($comparison->results as $row) {
            ...
            fputcsv($file, [
                ...
                $row->date_formatted ?? '',
                $row->description ?? '-',
                $row->bank_amount ? '$' . number_format($row->bank_amount, 2) : '',
                $row->ledger_date ?? '',
                ($row->ledger_description ?? '-') . ' (' . ($row->account_name ?? '') . ')',
                $row->ledger_amount ? '$' . number_format($row->ledger_amount, 2) : '',
 ...
 */
private function getLedgerTransactionsByDateRange($startDate, $endDate, $accountId = null)
{
    $query = TransactionLines::select(
            'transaction_lines.*',
    ...
    return $query->orderBy('transaction_lines.date', 'asc')->get();
}
...
public function accountTransactions($accountId, Request $request)
{
    try {
        ...
        // Get account details from BankAccount
        ...
        if ($account) {
            $accountName = $account->holder_name ?? $account->name ?? 'Account #' . $accountId;
        }
        ...
        return view('report.account_transactions', compact(
            'transactions', 
            'account', 
            'accountId', 
            'accountName',
        ...
        return redirect()->back()->with('error', __('Failed to load transactions: ') . $e->getMessage());
    }
}

/**
 * Compare bank vs ledger transactions
 */

public function bankStatementReconciliation(Request $request)
    {
        if (!\Auth::user()->can('statement report')) {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
        ...
        $statements = BankStatementSubmission::where('created_by', \Auth::user()->creatorId())
        ...
        $bankAccounts->prepend('All Accounts', '');
        ...
        $ledgerTransactions = $this->getLedgerTransactionsByDateRange($startDate, $endDate, $selectedAccount);
        
        // Calculate summary
        $summary = $this->calculateLedgerSummary($ledgerTransactions);
        
        return view('report.bank_statement_reconciliation', compact(
            'statements', 
            'bankAccounts', 
            'startDate', 
            'endDate', 
     ...
     */
    public function uploadBankStatementForReconciliation(Request $request)
    {
        $request->validate([
        ...
        try {
            ...
            $submission = BankStatementSubmission::create([
                ...
                'transactions' => [] // Will be populated after extraction
            ]);
            
            return redirect()
                ->route('report.bank.statement.reconciliation')
                ->with('success', __('Bank statement uploaded successfully.'));
     ...
     */
    public function compareBankStatement($id, Request $request)
    {
        $statement = BankStatementSubmission::where('id', $id)
            ->where('created_by', \Auth::user()->creatorId())
            ->firstOrFail();
        
        $startDate = $request->input('start_date', $statement->created_at->subDays(30)->format('Y-m-d'));
        $endDate = $request->input('end_date', $statement->created_at->format('Y-m-d'));
        $accountId = $request->input('account_id');
        
        // Get ledger transactions
        $ledgerTransactions = $this->getLedgerTransactionsByDateRange($startDate, $endDate, $accountId);
        $bankTransactions = $statement->transactions ?? [];
        
        // Compare transactions
        $comparison = $this->matchBankWithLedgerTransactions($bankTransactions, $ledgerTransactions);
        
        // Get accounts for dropdown
        $accounts = BankAccount::where('created_by', \Auth::user()->creatorId())->get();
        
        return view('report.bank_statement_comparison', compact(
            'statement', 
            'ledgerTransactions', 
            'bankTransactions', 
            'comparison',
    ...
    public function viewBankStatement($id)
    {
        ...
        return view('report.bank_statement_view', compact('statement', 'transactions', 'totalDebit', 'totalCredit'));
    }
    ...
    public function deleteBankStatement($id)
    {
        ...
        return redirect()
            ->route('report.bank.statement.reconciliation')
            ->with('success', __('Bank statement deleted successfully.'));
    }

    /**
     * Export Bank Statement Comparison
     */
    public function exportBankStatementComparison($id, Request $request)
    {
        $statement = BankStatementSubmission::where('id', $id)
            ->where('created_by', \Auth::user()->creatorId())
            ->firstOrFail();
        ...
        $filename = 'bank_statement_comparison_' . $statement->id . '_' . date('Y-m-d') . '.csv';
        ...
        $callback = function() use ($comparison, $statement, $startDate, $endDate) {
            $file = fopen('php://output', 'w');
            
            fputcsv($file, ['Bank Statement Comparison Report']);
            fputcsv($file, ['Statement ID:', $statement->id]);
            fputcsv($file, ['Bank Name:', $statement->bank_name ?? 'N/A']);
            fputcsv($file, ['Account Number:', $statement->account_number ?? 'N/A']);
            ...
            fputcsv($file, ['MATCHED TRANSACTIONS']);
            fputcsv($file, ['Date', 'Description', 'Bank Amount', 'Ledger Date', 'Ledger Description', 'Ledger Amount', 'Match Score']);
            
            foreach ($comparison['matched'] as $match) {
                $bankTx = $match['bank_transaction'];
                $ledgerTx = $match['ledger_transaction'];
                fputcsv($file, [
                    $bankTx['date'] ?? 'N/A',
                    $bankTx['description'] ?? 'N/A',
                    number_format(($bankTx['debit'] ?? 0) + ($bankTx['credit'] ?? 0), 2),
                    $ledgerTx->date ?? 'N/A',
                    $ledgerTx->description ?? 'N/A',
                    number_format(($ledgerTx->debit ?? 0) + ($ledgerTx->credit ?? 0), 2),
                    $match['match_score'] . '%'
                ]);
            }
     ...
     */
    private function calculateLedgerSummary($transactions)
    {
        return [
            'total_debit' => $transactions->sum('debit'),
            'total_credit' => $transactions->sum('credit'),
            'total_transactions' => $transactions->count(),
            'net_balance' => $transactions->sum('credit') - $transactions->sum('debit'),
        ];
    }
     ...
     */
    private function calculateTransactionMatchScore($bankAmount, $bankDate, $bankDescription, $ledgerAmount, $ledgerDate, $ledgerDescription)
    {
        $score = 0;
        ...
        $dateDiff = abs(strtotime($ledgerDate) - strtotime($bankDate)) / (60 * 60 * 24);
        if ($dateDiff == 0) {
            $score += 30;
        } elseif ($dateDiff <= 3) {
            $score += 20;
    ...
    //         $filter['startDateRange'] = $start;
    //         $filter['endDateRange'] = $end;
    //         return view('report.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     `                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                _summary', compact('filter', 'accountss', 'accounts'));
    //     } else {
    //         return redirect()->back()->with('error', __('Permission Denied.'));
    //     }
    // }
...
public function ledgerSummary(Request $request, $account = '')
{
    ...
    \Cache::forget('ledger_report_' . \Auth::id() . '_' . md5($request->getQueryString()));
    ...
    $selectedAccountId = $account ?: ($request->account ?? null);
    
    // ✅ FIX: Add missing $selectedAccount variable
    $selectedAccount = null;
    if ($selectedAccountId && $selectedAccountId != '0') {
        $selectedAccount = ChartOfAccount::where('created_by', \Auth::user()->creatorId())
            ->where('id', $selectedAccountId)
            ->first();
    }
    
    $bankSubmissionId = $request->bank_submission_id;
    ...
    // ALWAYS: Get ledger accounts & transactions (existing logic)
    $parentAccounts = ChartOfAccount::where('created_by', \Auth::user()->creatorId())
        ->where('parent', 0)
        ->with('subAccounts')
        ->get();
    ...
    foreach ($parentAccounts as $parent) {
        $allAccounts->push($parent);
        $allAccounts = $allAccounts->merge($parent->subAccounts ?? collect());
    }

    $accountsForFilter = $allAccounts->map(function ($acc) {
        return [
            'id' => $acc->id,
            'code' => $acc->code,
            'name' => $acc->name . ($acc->parent ? ' (Sub)' : ''),
        ];
    })->prepend(['id' => 0, 'code' => '', 'name' => 'All Accounts']);

    $targetAccounts = $selectedAccountId && $selectedAccountId != '0' 
        ? $allAccounts->where('id', $selectedAccountId)
        : $allAccounts;

    $accountData = []; // ✅ FIX: Initialize accountData array
    foreach ($targetAccounts as $account) {
        $rawData = \App\Models\Utility::getAccountData($account->id, $startDate, $endDate);
        $transactionCount = $rawData->count();

        $transactions = collect($rawData)
            ->map(function ($txn) {
                $txn->net = ($txn->debit ?? 0) - ($txn->credit ?? 0);
                return $txn;
            })
            ->sortByDesc('date');

        $totals = $transactions->reduce(function ($carry, $txn) {
            $carry['debit'] += $txn->debit ?? 0;
            $carry['credit'] += $txn->credit ?? 0;
            $carry['balance'] += $txn->net ?? 0;
            $carry['count'] = ($carry['count'] ?? 0) + 1;
            return $carry;
        }, ['debit' => 0, 'credit' => 0, 'balance' => 0, 'count' => 0]);

        $accountData[] = [  // ✅ FIX: Push to accountData array
            'account' => $account,
    ...
    $overallTotals = collect($accountData)->reduce(function ($carry, $data) {
        $carry['debit'] += $data['totals']['debit'] ?? 0;
        $carry['credit'] += $data['totals']['credit'] ?? 0;
        $carry['balance'] += $data['totals']['balance'] ?? 0;
        $carry['accounts'] = ($carry['accounts'] ?? 0) + 1;
        return $carry;
    }, ['debit' => 0, 'credit' => 0, 'balance' => 0, 'accounts' => 0]);

    $filter['totals'] = $overallTotals;

    // NEW: Bank Statement Comparison (only if bank_submission_id provided)
    $bankComparison = null;
    $bankSubmission = null;
    $recentSubmissions = [];

    if ($bankSubmissionId) {
        $bankSubmission = \App\Models\BankStatementSubmission::where('id', $bankSubmissionId)
            ->where('created_by', \Auth::user()->creatorId())
            ->first();

        if ($bankSubmission && $selectedAccountId) {
            // Get ledger transactions for comparison
            $ledgerTransactions = TransactionLines::select(
                'transaction_lines.*',
                'chart_of_accounts.name as account_name',
                'chart_of_accounts.code as account_code'
            )
            ->leftJoin('chart_of_accounts', 'transaction_lines.account_id', '=', 'chart_of_accounts.id')
            ->where('transaction_lines.account_id', $selectedAccountId)
            ->where('transaction_lines.created_by', \Auth::user()->creatorId())
            ->whereBetween('transaction_lines.date', [$startDate, $endDate])
            ->orderBy('transaction_lines.date', 'asc')
            ->get();

            $bankTransactions = $bankSubmission->transactions ?? [];
            
            // Reuse comparison logic from BankStatementController
            $bankComparison = $this->compareBankTransactions($bankTransactions, $ledgerTransactions);
        }
    }

    // Get recent bank submissions for dropdown
    $recentSubmissions = \App\Models\BankStatementSubmission::where('created_by', \Auth::user()->creatorId())
        ->latest()
        ->limit(10)
        ->get()
        ->map(function($s) {
            return [
                'id' => $s->id,
                'name' => $s->bank_name . ' - ' . $s->original_file_name . ' (' . $s->created_at->format('M d') . ')'
            ];
        });

    // ✅ FIX: Pass correct variables to view
    return view('bank-reconciliation.ledger-report', compact(
        'filter', 
        'accountData',      // ✅ FIX: Pass accountData (not 'accounts')
        'accountsForFilter',
        'bankComparison',
        'bankSubmission',
        'recentSubmissions',
        'selectedAccount',     // ✅ ADDED: Missing selectedAccount variable
        'selectedAccountId',
        'bankSubmissionId'
    ));
}
    ...
    /**
     * Route::get('/report/ledger/preview', [ReportController::class, 'ledgerPreview'])->name('report.ledger.preview');
     */
    public function ledgerPreview(Request $request)
{
    try {
        ...
        $transactions = TransactionLines::select(
            'transaction_lines.*',
            'chart_of_accounts.name as account_name',
            'chart_of_accounts.code as account_code'
        )
        ...
        // Get opening balance
        $openingBalance = TransactionLines::where('account_id', $accountId)
            ->where('created_by', \Auth::user()->creatorId())
        ...
        \Log::info('Ledger transactions fetched', [
...
//            -----------------------------
   ...
   /**
 * Bank Reconciliation Report - Compare bank statement with ledger
 */
/**
 * Bank Reconciliation Report - Compare bank statement with ledger
 */
public function bankReconciliation(Request $request)
{
    if (\Auth::user()->can('ledger report')) {
        ...
        // Get account filter from request (same as ledger report)
        $accountId = $request->input('account_id', null);

        if ($request->has('statement_id') && !empty($request->statement_id)) {
            $selectedStatement = BankStatementSubmission::where('id', $request->statement_id)
                ->where('created_by', \Auth::user()->creatorId())
                ->first();

            if ($selectedStatement) {
                $bankTransactions = $selectedStatement->transactions ?? [];

                if (!empty($request->start_date) && !empty($request->end_date)) {
                    $start = $request->start_date;
                    $end = $request->end_date;
                } else {
                    $start = $selectedStatement->created_at->copy()->startOfMonth()->format('Y-m-d');
                    $end = $selectedStatement->created_at->format('Y-m-d');
                }

                // Get ledger transactions with account filter (same as ledger report)
                $ledgerTransactions = $this->getLedgerTransactionsForReconciliation($start, $end, $accountId);
                
                $comparisonResult = $this->compareBankWithLedger($bankTransactions, $ledgerTransactions);
                
                // Format matched transactions for the view
                $matchedTransactionsList = [];
                foreach ($comparisonResult->results as $result) {
                    $bankTxn = null;
                    $ledgerTxn = null;
                    
                    foreach ($bankTransactions as $bt) {
                        if (($bt['date'] ?? '') == $result->date_formatted && 
                            abs(($bt['debit'] ?? 0) - ($bt['credit'] ?? 0)) == $result->bank_amount) {
                            $bankTxn = $bt;
                            break;
                        }
                    }
                    
                    foreach ($ledgerTransactions as $lt) {
                        if ($lt->date == $result->ledger_date && 
                            abs(($lt->debit ?? 0) - ($lt->credit ?? 0)) == $result->ledger_amount) {
                            $ledgerTxn = $lt;
                            break;
                        }
                    }
                    ...
                    $matchedTransactionsList[] = [
                        'bank_transaction' => $bankTxn ?: ['date' => $result->date_formatted, 'description' => $result->description],
                        'ledger_transaction' => $ledgerTxn,
                        'match_score' => $result->match_score,
                        'match_type' => $result->status == 'matched' ? 'exact' : 'amount_only',
                ...
                $matchedTransactions = [
                    'matched' => $matchedTransactionsList,
                    'total_bank_amount' => $comparisonResult->total_bank_txs,
                    'total_ledger_amount' => $comparisonResult->total_ledger_txs,
                    'difference' => $comparisonResult->total_variance,
                    'matched_amount' => $comparisonResult->matched_count,
                    'match_rate' => $comparisonResult->match_accuracy
                ];
                
                $unmatchedBank = $comparisonResult->unmatched_bank;
                $unmatchedLedger = $comparisonResult->unmatched_ledger;
            }
        }

        $filter['startDateRange'] = $request->start_date ?? date('Y-m-01');
        $filter['endDateRange'] = $request->end_date ?? date('Y-m-t');
        ...
        return view('report.bank_reconciliation', compact(
            'bankStatements',
            'selectedStatement',
            'bankTransactions',
            'ledgerTransactions',
...
/**
 * Get ledger transactions for reconciliation
 */
/**
 * Get ledger transactions for reconciliation - SAME LOGIC as ledgerSummary
 */
private function getLedgerTransactionsForReconciliation($startDate, $endDate, $accountId = null)
{
    $query = TransactionLines::select(
            'transaction_lines.*',
            'chart_of_accounts.name as account_name',
 ...
 */
public function bankReconciliationExport(Request $request)
{
    $statementId = $request->statement_id;
    $selectedStatement = BankStatementSubmission::where('id', $statementId)
        ->where('created_by', \Auth::user()->creatorId())
        ->first();

    if (!$selectedStatement) {
        return redirect()->back()->with('error', __('Bank statement not found.'));
    }

    $start = $request->start_date ?? $selectedStatement->created_at->copy()->startOfMonth()->format('Y-m-d');
    $end = $request->end_date ?? $selectedStatement->created_at->format('Y-m-d');

    $bankTransactions = $selectedStatement->transactions ?? [];
    $ledgerTransactions = $this->getLedgerTransactionsForReconciliation($start, $end);
    
    // Use compareBankWithLedger instead of matchBankWithLedger
    $comparisonResult = $this->compareBankWithLedger($bankTransactions, $ledgerTransactions);

    $companyName = User::where('id', \Auth::user()->creatorId())->first();
    $companyName = $companyName->name;

    $name = 'bank_reconciliation_' . date('Y-m-d') . '.xlsx';
    $data = Excel::download(new BankReconciliationExport($comparisonResult, $selectedStatement, $start, $end, $companyName), $name);
    ob_end_clean();
    ...
    public function PayablesReport(Request $request)
    {
        ...
        /**
     * Alias method for compareBankWithLedger - maintains backward compatibility
     * This prevents "method not found" errors without deleting existing code
     */
    private function matchBankWithLedger($bankTransactions, $ledgerTransactions)
    {
        return $this->compareBankWithLedger($bankTransactions, $ledgerTransactions);
    }

    private function matchBankWithLedgerTransactions($bankTransactions, $ledgerTransactions)
    {
        $comparison = $this->compareBankWithLedger($bankTransactions, $ledgerTransactions);
        
        // Convert the object format to array format expected by some methods
        return [
            'matched' => $comparison->results,
            'unmatched_bank' => $comparison->unmatched_bank,
            'unmatched_ledger' => $comparison->unmatched_ledger,
            'total_matched' => $comparison->matched_count,
            'total_bank' => $comparison->total_bank_txs,
            'total_ledger' => $comparison->total_ledger_txs,
            'total_bank_amount' => $comparison->total_bank_txs,
            'total_ledger_amount' => $comparison->total_ledger_txs,
            'total_matched_amount' => $comparison->matched_count,
            'difference' => $comparison->total_variance,
            'match_rate' => $comparison->match_accuracy
        ];
    }

    private function compareTransactions($bankTxns, $ledgerTxns)
    {
        return $this->compareBankWithLedger($bankTxns, $ledgerTxns);
    }

    private function compareBankTransactions($bankTxns, $ledgerTxns)
    {
        return $this->compareBankWithLedger($bankTxns, $ledgerTxns);
    }
}
