...
class ProjectTaskController extends Controller
{
    ...
    public function index($project_id)
    {
        ...
        if (\Auth::user()->can('manage project task')) {
            ...
            if ($project != null) {
                ...
                // Auto-create task stages if none exist
                if($stages->isEmpty()) {
                    Utility::project_task_stages(\Auth::user()->creatorId());
                    $stages = TaskStage::orderBy('order')->where('created_by', \Auth::user()->creatorId())->get();
                }
    ...
    public function store(Request $request, $project_id, $stage_id)
    {
        if (\Auth::user()->can('create project task')) {
            ...
            $usr = Auth::user();
            $project = Project::find($project_id);
            $last_stage = TaskStage::orderBy('order', 'DESC')->where('created_by', \Auth::user()->creatorId())->first()->id;
            $post = $request->all();
...
//                }
    ...
    public function commentDestroyFile(Request $request, $projectID, $taskID, $fileID)
    {
        if (\Auth::user()->can('view project task')) {
            ...
            $path = public_path('tasks/' . $commentFile->file);
            if (file_exists($path)) {
                \File::delete($path);
            }
            $commentFile->delete();
