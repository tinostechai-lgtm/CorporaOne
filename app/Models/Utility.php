...
class Utility extends Model
{
     ...
     * This is needed when settings are updated to force reload on next request
     */
    public static function clearStaticSettingsCache()
    {
...
{
    // Get the current user's creator ID (company ID)
    ...
    // Clear the cache so future reads get the new value
    self::clearStaticSettingsCache();
...
//        return ($taxRate / 100) * (($price-$discount) * $quantity);
    ...
    public static function bankAccountBalance($id, $amount, $type)
    {
        ...
        if ($bankAccount) {
            if ($type == 'credit') {
                $bankAccount->balance -= $amount; // Credit decreases balance
            } elseif ($type == 'debit') {
                $bankAccount->balance += $amount; // Debit increases balance
            }
            $bankAccount->save();
    
            // Log for debugging
            \Log::info("Account {$id}: {$type} of {$amount}, New Balance: {$bankAccount->balance}");
        }
    }
...
//        dd($msg);
    ...
    public static function colorset()
    {
        ...
        if ($user) {
            ...
            $setting = []; // Default empty array if user is null
        }

        // If 'color' is not set, get default settings
        if (!isset($setting['color'])) {
            $setting = Utility::settings();
        }
...
public static function get_file($path)
{
    ...
    try {
        if ($settings['storage_setting'] == 'wasabi') {
            ...
            // ✅ FIX: Check public path first
            $publicPath = public_path($path);
            if (file_exists($publicPath)) {
                return asset($path);
            }
            
            // Check via storage symlink
    ...
    public static function getTargetrating($designationid, $competencyCount)
    {
        if (self::$getRatingData == null) {
            ...
            if (!empty($indicator->rating) && ($competencyCount != 0)) {
                ...
                // Check if rating is a valid array before using array_sum
    ...
    {
        $setting = Utility::settings();
        $path = public_path($setting['google_calender_json_file']);
        config([
    ...
    public static function changeStorageLimit($company_id, $file_path)
    {
        ...
        $files = \File::glob(public_path($file_path));
        $fileSize = 0;
    ...
    public static function getAccountData($account_id, $start_date = null, $end_date = null)
    {
        \Log::info('getAccountData called', ['account_id' => $account_id, 'start_date' => $start_date, 'end_date' => $end_date]);
        ...
        // Primary: TransactionLines with recent-first ordering
        $transactionData = DB::table('transaction_lines')
            ...
            ->leftJoin('invoices', function ($join) {
                $join->on('transaction_lines.reference_id', '=', 'invoices.id')
                    ->whereIn('transaction_lines.reference', ['Invoice', 'Invoice Payment']);
            })
            ...
            ->leftJoin('payments', function ($join) {
                $join->on('transaction_lines.reference_id', '=', 'payments.id')
                    ->where('transaction_lines.reference', 'Payment');
            })
            ...
            ->select(
                ...
                DB::raw("COALESCE(customers.name, venders.name, revenues_customers.name, payments_venders.name, 'Direct Transaction') as user_name"),
                DB::raw("COALESCE(invoices.invoice_id, bills.bill_id, revenues.id, payments.id) as ids"),
                'transaction_lines.date',
                'transaction_lines.created_at'
            )
            ->orderBy('transaction_lines.date', 'DESC')
            ->orderBy('transaction_lines.created_at', 'DESC')
            ->get();

        \Log::info('getAccountData results', ['count' => $transactionData->count(), 'first_date' => $transactionData->first()?->date ?? 'none']);
    ...
    {
        // FIX: Skip if account_id is null/empty - prevents DB constraint violation
        if (empty($data['account_id']) || $data['account_id'] == 0) {
            \Log::warning('TransactionLines skipped: NULL account_id', [
                'reference' => $data['reference'] ?? 'N/A',
                'reference_id' => $data['reference_id'] ?? 'N/A',
                'reference_sub_id' => $data['reference_sub_id'] ?? 'N/A',
                'data' => $data
            ]);
            return; // Skip creation/update
        }
    ...
    // app/Models/Utility.php - Add these methods

public static function getOfficeLocation()
{
    $settings = self::settings();
    return [
        'latitude' => $settings['office_latitude'] ?? null,
        'longitude' => $settings['office_longitude'] ?? null,
        'radius' => $settings['office_radius'] ?? 300,
        'address' => $settings['office_address'] ?? null,
        'restriction_enabled' => $settings['location_restriction'] ?? false,
    ];
}
