                    ...
                    a('[data-plugin="dragula"]').each(function () {
                        ...
                        r ? dragula(n, {
                            ...
                            $.ajax({
                                ...
                                success: function (data) {
                                    show_toastr('success', 'Job-application successfully updated', 'success');
                                },
                                error: function (data) {
                    ...
                    <div class="card-body">
                        ...
                        <div class="row d-flex align-items-center justify-content-end">
                            ...
                            <div class="col-auto float-end ms-2 mt-4">
                                <a href="#" class="btn btn-sm btn-primary me-1" onclick="document.getElementById('applicarion_filter').submit(); return false;" data-bs-toggle="tooltip" data-bs-original-title="{{ __('Apply') }}">
                                    <span class="btn-inner--icon"><i class="ti ti-search"></i></span>
                                </a>
                                <a href="{{ route('job-application.index') }}" class="btn btn-sm btn-danger" data-bs-toggle="tooltip" title="{{ __('Reset') }}">
                                                    ...
                                                    <div class="btn-group card-option">
                                                        ...
                                                        <div class="dropdown-menu dropdown-menu-end">
                                                            @can('show job application')
                                                                <a class="dropdown-item" href="{{ route('job-application.show', \Crypt::encrypt($application->id)) }}">{{ __('View') }}</a>
                                                            @endcan
                                                            @can('delete job application')
                                                                ...
                                                                <a href="#!" class="dropdown-item bs-pass-para"><i class="ti ti-archive"></i> {{ __('Delete') }}</a>
                                                                {!! Form::close() !!}
                                                            @endcan
                                                        </div>
                                                    </div>
                                        ...
                                        <div class="card-body">
                                            <div class="d-flex align-items-center justify-content-between">
                                                <ul class="list-inline mb-0 mt-0">
                                                    ...
                                                    <li class="list-inline-item d-inline-flex align-items-center" data-bs-toggle="tooltip" title="{{ __('Applied at') }}">
                                                        <i class="ti ti-clock me-1"></i>{{ \Auth::user()->dateFormat($application->created_at) }}
                                                    </li>
                                                </ul>
                                                <div class="user-group">
                                                    <img src="{{ !empty($application->profile) ? asset('uploads/job_application/profile/' . basename($application->profile)) : asset('Uploads/avatar/avatar.png') }}"
                                                         class="hweb rounded border-2 border border-primary" alt="Profile Image">
                                                    @if(!empty($application->profile) && !file_exists(public_path('Uploads/job_application/profile/' . $application->profile)))
                                                        <!-- Debug: Log missing file -->
                                                        <span class="text-danger d-none">{{ 'Missing file: ' . $application->profile }}</span>
                                                    @endif
                                                </div>
                                            </div>
                                        </div>
