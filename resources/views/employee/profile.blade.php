...
@php
    // ✅ Define avatar path helper - KEEP THIS ONLY ONCE
...
@section('content')
    <div class="row">
        @forelse($employees as $employee)
            @php
                // ✅ FIXED: Use the existing $avatarPath variable, don't redeclare it
                $avatarFile = $employee->user->avatar ?? 'avatar.png';
                // Use the $avatarPath from the top of the file
                $avatarUrl = $avatarPath . $avatarFile;
                
                // Optional: Check if file exists, fallback to default
                // If using Utility::get_file(), it already handles fallback
                // But for extra safety:
                if (!file_exists(public_path('uploads/avatar/' . $avatarFile))) {
                    $avatarUrl = $avatarPath . 'avatar.png';
                }
            @endphp
            <div class="col-lg-3 col-sm-6 col-md-6">
                <div class="card profile-card">
                    <div class="avatar-parent-child">
                        {{-- ✅ FIXED: Use the correct avatar URL --}}
                        <img src="{{ $avatarUrl }}" class="avatar rounded-circle avatar-xl" alt="{{ $employee->name }}">
                    </div>
