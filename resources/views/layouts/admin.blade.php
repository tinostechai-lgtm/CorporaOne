...
<head>
    ...
    <link rel="icon"
    href="{{ asset('storage/uploads/logo/' . ($company_favicon ?? 'favicon.png')) }}"
    type="image/png" sizes="16x16">


    <!-- Favicon icon -->
    {{--    <link rel="icon" href="/assets/images/13.png" type="image/x-icon" /> --}}
    <!-- Calendar-->
...
<body class="{{ $themeColor }}">
    ...
    <!-- ========================================== -->
    <!--  PRELOADER BLOCK – COMPLETELY REMOVED      -->
    <!--  (The old <div class="loader-bg"> is GONE) -->
    <!-- ========================================== -->
...
<script>
    ...
    // Function to update logo cache on all pages after settings change
    function refreshLogoCache() {
        var timestamp = new Date().getTime();
        
        // Update all logo images with cache-busting
        var logos = document.querySelectorAll('img[src*="uploads/logo"]');
        ...
        if (favicon) {
            var faviconSrc = favicon.href.split('?v=')[0];
            favicon.href = faviconSrc + '?v=' + timestamp;
        }
    }

    // Listen for settings form submissions and refresh logos after save
    document.addEventListener('DOMContentLoaded', function() {
        // Find all forms in settings pages
        var settingsForms = document.querySelectorAll('form');
        settingsForms.forEach(function(form) {
            var action = form.getAttribute('action') || '';
            if (action.includes('setting') || action.includes('settings') || action.includes('company')) {
                form.addEventListener('submit', function(e) {
                    // Store flag to refresh after form submission
                    sessionStorage.setItem('refreshLogos', 'true');
                });
            }
        });

        // Check if we need to refresh logos after page load
        if (sessionStorage.getItem('refreshLogos') === 'true') {
            ...
            setTimeout(function() {
                ...
                if (typeof toastr !== 'undefined') {
                    toastr.success('Settings saved successfully. Logo updated.');
                }
            }, 500);
        }
    });

    // Also handle AJAX-based form submissions
    $(document).ajaxComplete(function(event, xhr, settings) {
        if (xhr.responseJSON && (xhr.responseJSON.success || xhr.responseJSON.message)) {
            if (settings.url.includes('setting') || settings.url.includes('settings') || settings.url.includes('company')) {
                setTimeout(function() {
                    refreshLogoCache();
                }, 500);
            }
