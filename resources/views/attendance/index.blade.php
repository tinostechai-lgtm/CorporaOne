...
@section('content')
                    ...
                    <div class="card-body">
                        ...
                        <div class="row align-items-center justify-content-end">
                            ...
                            <div class="col-auto mt-4">
                                <div class="row">
                                    <div class="col-auto">
                                        <a href="#" class="btn btn-sm btn-primary me-1" onclick="document.getElementById('attendanceemployee_filter').submit(); return false;" data-bs-toggle="tooltip" title="{{__('Apply')}}" data-original-title="{{__('apply')}}">
    ...
    <tr>
        ...
        <th>{{ __('Status') }}</th>
        <th>{{ __('Clock In') }}</th>
        <th>{{ __('Tea Break Out') }}</th>   {{-- NEW --}}
        <th>{{ __('Tea Break In') }}</th>    {{-- NEW --}}
        <th>{{ __('Clock Out') }}</th>
        <th>{{ __('Late') }}</th>
...
<tbody>
    @foreach ($attendanceEmployee as $attendance)
        <tr>
            ...
            <td>
                <span class="badge rounded p-2 px-3
                    @if($attendance->status == 'Present') bg-success
                    @elseif($attendance->status == 'Leave') bg-danger
                    @else bg-warning @endif">
                    {{ __($attendance->status) }}
                </span>
            </td>
            <td>{{ ($attendance->clock_in != '00:00:00') ? \Auth::user()->timeFormat($attendance->clock_in) : '—' }}</td>
            
            {{-- Tea Break Out --}}
            <td>
                @if($attendance->tea_break_out && $attendance->tea_break_out != '00:00:00')
                    {{ \Auth::user()->timeFormat($attendance->tea_break_out) }}
                @else
                    —
                @endif
            </td>
            ...
            <td>
                @if($attendance->tea_break_in && $attendance->tea_break_in != '00:00:00')
                    {{ \Auth::user()->timeFormat($attendance->tea_break_in) }}
                @else
                    —
                @endif
            </td>
            
            <td>{{ ($attendance->clock_out != '00:00:00') ? \Auth::user()->timeFormat($attendance->clock_out) : '—' }}</td>
                ...
                <td>
                    @can('edit attendance')
                        <div class="action-btn me-2">
                            <a href="#" data-url="{{ URL::to('attendanceemployee/'.$attendance->id.'/edit') }}"
